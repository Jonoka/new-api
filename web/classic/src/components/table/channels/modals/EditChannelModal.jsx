  ...
  const loadChannel = async () => {
    ...
    if (success) {
      ...
      if (data.settings) {
        try {
          ...
          data.monitor_response_time_threshold_seconds =
            typeof parsedSettings.monitor_response_time_threshold_seconds ===
            'number'
      ...
      const hasAdvancedValues =
        ...
        data.monitor_enabled !== 'inherit' ||
        (data.monitor_test_interval_minutes &&
          data.monitor_test_interval_minutes.trim()) ||
        (data.monitor_response_time_threshold_seconds &&
          data.monitor_response_time_threshold_seconds.trim()) ||
        data.monitor_auto_disable_enabled !== 'inherit' ||
        data.monitor_auto_enable_enabled !== 'inherit' ||
        (data.monitor_disable_threshold &&
          data.monitor_disable_threshold.trim()) ||
                ...
                <div className='pt-3'>
                  ...
                  {inputs.type === 14 && (
                    <>
                      ...
                      />
                      <Form.Switch
                        field='claude_code_fingerprint_enabled'
                        label={t('Claude Code 指纹访问')}
                        checkedText={t('开')}
                        uncheckedText={t('关')}
                        onChange={(value) =>
                          handleChannelOtherSettingsChange(
                        ...
                        extraText={t(
                          '开启后，该渠道请求 Claude 时会注入 Claude Code 请求头、系统标识和 metadata；Header Override 仍可覆盖默认指纹头',
                        )}
                      />
                      <Form.Switch
                        field='claude_code_transport_fingerprint_enabled'
                        label={t('Claude Code Transport 指纹')}
                        checkedText={t('开')}
                        uncheckedText={t('关')}
                        onChange={(value) =>
                          handleChannelOtherSettingsChange(
                        ...
                        extraText={t(
                          '开启后，该渠道请求 Claude 时会使用内置 uTLS Claude Code Node24 Transport 指纹；不改变 Header Override 优先级',
                        )}
                      />
                      <Form.Input
                        field='claude_code_version'
                        label={t('Claude Code 版本号')}
                        placeholder='2.1.156'
                        onChange={(value) =>
                          handleChannelOtherSettingsChange(
                            'claude_code_version',
                        ...
                        extraText={t(
                          '自定义 User-Agent 中的 Claude Code 版本号，留空使用默认值',
                        )}
                      />
                    </>
                  )}
                  ...
                  <div className='p-2 space-y-3' ref={formContainerRef}>
                    ...
                    <Card className='!rounded-2xl shadow-sm border-0'>
                      ...
                      <Form.Select
                        ...
                        rules={[
                          { required: true, message: t('请选择协议类型') },
                        ]}
                      ...
                      <Form.Select
                        ...
                        onChange={(value) =>
                          handleInputChange('vendor_id', value || 0)
                        }
