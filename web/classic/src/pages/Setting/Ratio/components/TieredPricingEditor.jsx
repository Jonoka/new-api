...
export default function TieredPricingEditor({
  ...
  const [editorMode, setEditorMode] = useState(() =>
    tryParseRequestRuleExpr(requestRuleExpr || '') !== null &&
  ...
  const [rawExpr, setRawExpr] = useState(() =>
    combineBillingExpr(currentExpr, requestRuleExpr || ''),
  );
  ...
  const handleRequestRuleGroupsChange = useCallback(
    (nextGroups) => {
      ...
      if (areRequestRuleGroupsValid(nextGroups)) {
  ...
  useEffect(() => {
    ...
    if (editorMode === 'raw') {
      const { requestRuleExpr: ruleExpr } =
  ...
  return (
    <div>
      ...
      {editorMode === 'visual' && !validRuleGroups && (
        <Banner type='danger' description={t('请求规则无效')} />
      )}
