...
export default function GroupRatioSettings(props) {
  ...
  const syncGroupsState = useCallback((nextGroups) => {
    ...
    if (previousCodes !== nextCodes) {
      setAutoListVersion((version) => version + 1);
    }
  }, []);
  ...
  const fetchGroupDetails = useCallback(async () => {
    ...
    const responseGroups = extractGroupDetailsResponse(response?.data);
  ...
  }, [commitGroupDetails, fetchGroupDetails, t]);
  ...
  async function onSubmit() {
    ...
    const updateArray = compareObjects(inputs, inputsRow).filter((item) =>
      LEGACY_OPTION_KEYS.includes(item.key),
    );
    const deletedIds = getDeletedGroupIds(
      originalGroupsRef.current,
      groupsRef.current,
    );
    const groupPayload = buildGroupDetailsPayload(
      groupsRef.current,
      deletedIds,
    );
    const originalPayload = buildGroupDetailsPayload(
      originalGroupsRef.current,
      [],
    );
    const groupsChanged =
    ...
    groupPayload.groups.forEach((group) => {
      codeCounts.set(group.code, (codeCounts.get(group.code) || 0) + 1);
    });
  ...
  const handleAutoGroupsChange = useCallback(
    (value) => {
      const knownCodes = new Set(
        groupsRef.current.map((group) => String(group.code || '').trim()),
      );
      const requestedCodes = parseAutoGroupCodes(value);
      const seenCodes = new Set();
      const autoCodes = requestedCodes.filter((code) => {
        if (!knownCodes.has(code) || seenCodes.has(code)) return false;
        seenCodes.add(code);
        return true;
      });
      syncGroupsState(applyAutoGroupCodes(groupsRef.current, autoCodes));
      if (autoCodes.length !== requestedCodes.length) {
        setAutoListVersion((version) => version + 1);
      }
    },
    [syncGroupsState],
  ...
  const groupDv = groupDataVersionRef.current;
