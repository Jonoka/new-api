...
import { embeddedContentIframeSandbox } from '@/lib/content-format'
...
// 隔离模式允许样式和多媒体，但明确禁止可执行标签及 iframe srcdoc。
...
function hardenIsolatedHtml(html: string): string {
  ...
  template.content.querySelectorAll('iframe').forEach((frame) => {
    ...
    frame.setAttribute('sandbox', embeddedContentIframeSandbox)
