...
test('login return survives authentication once, expires and never stores arbitrary navigation', () => {
  ...
  rememberCanvasReturn('/canvas?canvas_resume=1&group=vip&canvas_next=%2Fprojects%2F1&role=100');
  const result = new URL(consumeCanvasReturn(), 'https://api.test');
  assert.equal(result.pathname, '/canvas');
  ...
  rememberCanvasReturn('/canvas?canvas_resume=1');
  for (const [key, value] of storage) storage.set(key, JSON.stringify({ ...JSON.parse(value), expires: 0 }));
  assert.equal(consumeCanvasReturn(), '');
  for (const target of ['/dashboard?canvas_resume=1', '//evil.test/canvas?canvas_resume=1', '/canvas', 'https://evil.test']) {
