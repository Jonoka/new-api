...
func TestConvertImageTaskResponseToB64IfRequestedConvertsImageEditNestedResult(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "image/png")
		_, _ = w.Write([]byte("edit-poll-image"))
	}))
	defer server.Close()
	...
	body := []byte(`{"status":"succeeded","result":{"data":[{"height":1024,"url":"` + server.URL + `/img.png","width":1024}]}}`)
	...
	data := result["data"].([]any)
	item := data[0].(map[string]any)
	assert.Equal(t, "ZWRpdC1wb2xsLWltYWdl", item["b64_json"])
	_, hasURL := item["url"]
	assert.False(t, hasURL)
}
