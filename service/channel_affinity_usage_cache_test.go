...
func channelAffinityUsageCacheTestID(t *testing.T) string {
	t.Helper()
	return fmt.Sprintf("%s_%d_%d_%d", t.Name(), os.Getpid(), time.Now().UnixNano(), channelAffinityUsageCacheTestSeq.Add(1))
}
...
func TestObserveChannelAffinityUsageCacheByRelayFormat_ClaudeMode(t *testing.T) {
	id := channelAffinityUsageCacheTestID(t)
	ruleName := fmt.Sprintf("rule_%s", id)
	usingGroup := "default"
	keyFP := fmt.Sprintf("fp_%s", id)
