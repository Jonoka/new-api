...
- 发票中心：支付时申请、历史订单合并申请、服务费支付、个人/企业和普票/专票状态流转。订单首次进入待开票状态产生 invoice_pending。代码见 model/invoice.go、model/invoice_order.go、model/invoice_payment.go。
...
- 渠道运维：趋势、稳定性、状态码、失败链路、并发限制、亲和缓存和上游模型变更检测。
